...
class LoadUserData extends AbstractFixture implements OrderedFixtureInterface, ContainerAwareInterface
{
    ...
    {
        $user = $this->container->get('doctrine')->getRepository('DFPEtapIBundle:Uzytkownik')->findOneBy(array('email'=>'akrolikowski@csv.pl'));
        if(!$user)
        {
        ...
        foreach ($yml as $u)
        {
            $user = $this->container->get('doctrine')->getRepository('DFPEtapIBundle:Uzytkownik')->findOneBy(array('email'=>$u['Email']));
            if(!$user)
            {
